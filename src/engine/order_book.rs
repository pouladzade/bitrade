    ...
    match order.side {
        OrderSide::Buy => {
            ...
            while let Some(mut ask) = self.asks.pop() {
                ...
                // Execute the trade
                let trade = Self::execute_trade(&mut order, &mut ask, trade_amount, trade_price);
                trades.push(trade);
        ...
        OrderSide::Sell => {
            ...
            while let Some(mut bid) = self.bids.pop() {
                ...
                // Execute the trade
                let trade = Self::execute_trade(&mut order, &mut bid, trade_amount, trade_price);
                trades.push(trade);
...
///
    fn execute_trade(
        ...
        Trade {
            ...
            amount,
            quote_amount,
            ask_user_id: counter_order.user_id,
            ask_order_id: counter_order.id,
            ask_role: MarketRole::Maker,
            ask_fee: counter_order.taker_fee * amount,
            bid_user_id: order.user_id,
            bid_order_id: order.id,
            bid_role: MarketRole::Taker,
            bid_fee: order.taker_fee * amount,
            ask_order: None,
            bid_order: None,
            state_before: None,
            state_after: None,
...
mod tests {
    ...
    fn create_order(id: u64, side: OrderSide, price: &str, amount: &str, create_time: f64) -> Order {
        Order {
            id,
            base_asset: "BTC".into(),
